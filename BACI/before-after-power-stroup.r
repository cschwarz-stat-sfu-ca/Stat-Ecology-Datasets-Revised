...
# 2020-05-08 CJS Updated names for power to resolve confusion about 2 sided and 1 sided power.
...
# What is the denominator df for the hypothesis test. See notes above
  dfdenom <- length(unique(Xvalues))-2 # approximation to df with multiple years in each of before and after
  if(Process.SD ==0){ dfdenom = length(Xvalues)-2}

  Fcrit <- qf(1-alpha, 1, dfdenom)
  power.2s <- 1 - pf(Fcrit, 1, dfdenom,ncp) # two sided power
...
#
  ...
  power.1s.a <- 1-pt(Tcrit,dfdenom,sqrt(ncp)) # one sided power
  power.1s.b <- pt(-Tcrit,dfdenom,sqrt(ncp))  # one sided power
