...
baci.power <- function(n_IA,n_IB,n_CA,n_CB,ns_I,ns_C,ny_B,ny_A,mu_IA,mu_IB,mu_CA,mu_CB,
...
#  Total number ob observations
   n <- ns_I*ny_A*n_IA+   # treatment sites x years after  x traps/site/year
        ns_I*ny_B*n_IB+   # treatment sites x years before x traps/site/year
   ...
   time <- treatment <- sites <- years <- mu <- rep(0,n)
	...
	for(site in 1:ns_I)
		{
		 for(year in 1:ny_A)
	         for(quadrat in 1:n_IA)
	            {	
	            	sites[m] <- paste("LT",site,sep="")
	            	years[m] <- paste("IA",year,sep="")
	            	treatment[m] <- "I"
	            	time[m] <- "A"
	            	mu[m] <- mu_IA
	            	m <- m+1
		  ...
		  for(year in 1:ny_B)
		     for(quadrat in 1:n_IB)
		     	{
		     		sites[m] <- paste("LT",site,sep="")
	            	years[m] <- paste("IB",year,sep="")
		     		treatment[m] <- "I"
	            	time[m] <- "B"
	            	mu[m] <- mu_IB
	            	m <- m+1
...
# We don't actually need the results, but we want the various matrices
  fit <- lme4::lmer(mu ~ -1 + CI:BA + (1|L) + (1|Time) + (1|L:Time), data = testdata,
  ...
  n <- nrow(testdata)
  nL <- ns_I+ns_C     # number of sites for the control and treatment
...
# the contrast vector for the BACI effect
  K <- c(1,-1,-1,1)
  baci=mu_IA-mu_IB-mu_CA+mu_CB
  
  # compute se of baci effect
  baci.se <-  as.numeric(sqrt(t(K)%*% solve( t(X)%*%solve(V)%*%X) %*%K)) 
...
#  calculate the non-centrality parameter, and then the power
  ...
  dfdenom <- (ny_B-1 + ny_A-1)*(ns_I-1 + ns_C-1) +  # site*time(BACI) term
             (ny_B-1 + ny_A-1)*1 +                  # BA*site(CI) term
             (ns_I-1 + ns_C-1)*1                    # CI*time(BA) term
...
#
  ...
  return(data.frame(alpha=alpha, 
     ...
     n_IA=n_IA, n_IB=n_IB, n_CA=n_CA,n_CB=n_CB,
     ns_I=ns_I, ns_C=ns_C, ny_B=ny_B, ny_A=ny_A, 
     mu_IA=mu_IA, mu_IB=mu_IB, mu_CA=mu_CA, mu_CB=mu_CB, 
     baci=baci, baci.se=baci.se,
